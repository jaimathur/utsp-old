...
$file = fopen($folder."SigGravModReport.xls", "a+");   
fclose($file);

$file1 = fopen($folder."CostFile.xls", "w");
$url = DOC_FOLDER."/costmatrix.xls"; 
...
$url = DOC_FOLDER."/Origin_gravity.xls"; 
//$m_TripFile = basename($url);
...
$url = DOC_FOLDER."/destination_gravity.xls"; 
//$m_TripFile = basename($url);
...
function chk1()
{
		...
		document.Frm.action="SigGravModDefaultInput.php";	
}
...
//------------------------------- Reading Xls file ----------------------------------------
	...
	<?php
		...
		if( !($OriginCol == $CostCol))
		{
   		...
   		echo '<div id="OriTot">';
   		echo '<caption><b> Future Year Origins Total </b></caption>';
		...
		echo '<div id="DestTot">';
		echo '<caption><b> Future Year Destinations Total </b></caption>';
        echo '<div id="scroller"><table class="table table-bordered table-hover">';
        echo'<tr align="center" ><td><b>Zone</b></td>';
        for ($i = 1; $i <= $n; $i++)
        {
