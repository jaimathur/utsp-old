...
include_once("../sessioncheck.php");
  ...
  getHeader(2,"Calibration of Doubly Constrained Gravity Model","Trip Distribution");
?> 
<div id="body">
...
<p class="section">Calibration is an important step as it estimates the value of 
	<strong>&alpha; or &beta; or c</strong> depending on the friction factor function  in the gravity equation given below such that the 
...
<span class="title">Calibration of  Doubly Constrained Gravity Model</span>
<p class="section">As the name suggests Doubly Constraint Gravity Model is a model where 
	both the Trip Production and Trip Attraction are constrained. </p>
$$ T_{ij} =X_{i}P_{i}Y_{i}A_{j}d_{ij}^\alpha $$
<div>$ X_{i}$ = Trip production balancing factor</div>
<div>$ Y_{j}$ = Trip attraction balancing factor</div>
<div>$d_{ij}^\alpha $ = distance or the travel time between zones where α is the parameter to be estimated</div>
$$ X_{i} = \dfrac{1}{\sum \limits_{j}{Y_{j}A_{j}d_{ij}^\alpha }}$$
$$ Y_{i} =\dfrac{1}{\sum \limits_{j}{X_{i}P_{i}d_{ij}^\alpha }}$$

The steps involved for calibration of doubly constraint model is given below:
<ul class="test1">
<li>Assume a trial value of &alpha;.</li>
<li>Take $ X_{i}$ for example as 1 and determine the value of $ Y_{j}$ from equation (3). 
	Then this value is substituted in equation (2) to find the value of $X_{i}$.</li>
<li>$ T_{ij}$ is calculated using the equation (1) and the above obtained values of $ X_{i}$ and $ Y_{j}$.</li>
<li>The result obtained can be represented in the form of a matrix.</li>
<li>A series of Gravity Model is obtained by varying the value of &alpha;. For each model a 
	degree of closeness is calculated which
 measures the difference between the calculated and observed value..</li>
</ul>
